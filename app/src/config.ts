...
export const SERVICE_NAME = {
  /** Workspace-level service: the primary context created during namespace bootstrap.
   *  Falls back to services[0] when no entry with id="directory" is declared
   *  (single-service apps where the primary service doubles as the workspace context). */
  get directory(): string {
    const svc = byId('directory') ?? config.services[0];
    if (!svc) throw new Error('studio.config.json: no services defined');
    return svc.name;
  },
