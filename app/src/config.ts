...
export const SERVICE_NAME = {
  /** Workspace-level service. Falls back to the first declared service when no
   *  entry with id="directory" exists (single-service specs). */
  get directory(): string {
    const svc = byId('directory') ?? config.services[0];
    if (!svc) throw new Error('studio.config.json: services[] is empty');
    return svc.name;
  },
  /** Per-context service: null when not declared (single-service specs). */
