 ...
 *
 *  Single-service fallback: when there is exactly one service and no entry has
 *  `id === 'directory'`, that single service IS the directory (1-service apps). */
function requireService(id: string): string {
  const svc = byId(id);
  if (svc) return svc.name;
  // Single-service apps: the only service acts as the directory context.
