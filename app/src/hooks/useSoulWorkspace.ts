...
export interface WorkspaceRecord {
  namespaceId: string;
  soulContextId: string | null;
  applicationId: string;
  alias?: string;
}

export interface UseSoulWorkspaceReturn {
  ...
  /** Executor public key for the selected soul context. */
...
export function useSoulWorkspace(): UseSoulWorkspaceReturn {
  ...
  // Resolve the soul context for the selected namespace.
  // `useGroupContexts` returns only {contextId, alias} — no serviceName.
  // We call mero.admin.getContext() for each and pick the one with PRIMARY_SERVICE_NAME.
  const [soulContextId, setSoulContextId] = useState<string | null>(null);

  useEffect(() => {
    if (!mero || namespaceContexts.length === 0) {
      setSoulContextId(null);
      return;
    }
    let cancelled = false;
    (async () => {
      try {
        ...
        if (cancelled) return;
        const match = details.find((d) => d.serviceName === PRIMARY_SERVICE_NAME)?.contextId ?? null;
        setSoulContextId(match);
      } catch {
        if (!cancelled) setSoulContextId(null);
      }
    })();
  ...
  const workspaces: WorkspaceRecord[] = useMemo(
    () =>
      namespaces.map((ns) => ({
        ...
        soulContextId: ns.namespaceId === selectedNsId ? soulContextId : null,
        applicationId: ns.targetApplicationId,
        alias: ns.alias,
      })),
    [namespaces, selectedNsId, soulContextId],
  );

  const selectedWorkspace = workspaces.find((w) => w.namespaceId === selectedNsId) ?? null;

  // --- Members (custom, bypasses SDK bug where response.data is read instead of {members, selfIdentity}) ---
  ...
  // --- Workspace ready state & executor key ---
  ...
  // Reset state on namespace change
  useEffect(() => {
    setWorkspaceReady(false);
    setExecutorPublicKey(null);
  }, [selectedNsId]);

  // Resolve executor identity for the soul context
  useEffect(() => {
    if (!soulContextId || !mero) return;
    let cancelled = false;
    (async () => {
      try {
        const { identities } = await mero.admin.getContextIdentitiesOwned(soulContextId);
    ...
    return () => { cancelled = true; };
  }, [soulContextId, mero, contextIdentity]);

  // Mark workspace ready once we have both contextId and executor key
  useEffect(() => {
    if (soulContextId && executorPublicKey && !workspaceReady) setWorkspaceReady(true);
  }, [soulContextId, executorPublicKey, workspaceReady]);
