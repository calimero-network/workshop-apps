...
import { SERVICE_NAME, PRIMARY_SERVICE_NAME, SELECTED_NAMESPACE_KEY, DEFAULT_WORKSPACE_NAME } from '../config';
import { TableClient } from '../api/table/TableClient';

/**
 * Resolve the service name to look up when finding the workspace context.
 * Tries SERVICE_NAME.directory; falls back to PRIMARY_SERVICE_NAME for
 * single-service apps where no 'directory' role is declared.
 */
function getWorkspaceServiceName(): string {
  try {
...
export function useChatLobby(): UseChatLobbyReturn {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const wsServiceName = getWorkspaceServiceName();
        const lobby = details.find((d) => d.serviceName === wsServiceName)?.contextId ?? null;
  ...
  const createLobby = useCallback(async (name?: string) => {
    const gameName = name || DEFAULT_WORKSPACE_NAME;
    const result = await createNamespaceWithLobby(gameName);
    if (result) {
      // For single-service apps the table context is created by bootstrap.
      // Call create_game so the DM sets the game name and transitions out of
      // the "lobby" placeholder state created by init().
      if (mero) {
        try {
          const tableClient = new TableClient(mero, result.lobbyContextId, result.memberPublicKey);
          await tableClient.createGame({ name: gameName });
        } catch (err) {
          // Non-fatal: DM can call create_game again from the table view.
          console.warn('create_game after bootstrap failed (will retry):', err);
        }
      }
