...
export interface UseTripWorkspaceReturn {
  ...
  selectedWorkspace: TripWorkspaceRecord | null;
...
export function useTripWorkspace(): UseTripWorkspaceReturn {
  ...
  // Resolve the trip context by serviceName (not by index — index 0 is unsafe).
  const [tripContextId, setTripContextId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setTripContextId(match);
      } catch {
        if (!cancelled) setTripContextId(null);
      }
    })();
    return () => { cancelled = true; };
  }, [mero, namespaceContexts]);

  const workspaces: TripWorkspaceRecord[] = useMemo(
    () => namespaces.map((ns) => ({
      namespaceId: ns.namespaceId,
      tripContextId: ns.namespaceId === selectedNsId ? tripContextId : null,
      applicationId: ns.targetApplicationId,
      alias: ns.alias,
    })),
    [namespaces, selectedNsId, tripContextId],
  );
  ...
  // Members — SDK bug workaround: response is {members, selfIdentity}, not .data
  ...
  const refetchMembers = useCallback(async () => {
    ...
    } catch { /* leave previous state on transient errors */ }
  ...
  // Resolve executor identity for the trip context
  useEffect(() => {
    if (!tripContextId || !mero) return;
    let cancelled = false;
    (async () => {
      try {
    ...
    return () => { cancelled = true; };
  }, [tripContextId, mero, contextIdentity]);

  // Mark workspace as joined once we have identity
  useEffect(() => {
    if (tripContextId && executorPublicKey && !workspaceJoined) {
      setWorkspaceJoined(true);
    }
  }, [tripContextId, executorPublicKey, workspaceJoined]);
  ...
  const createWorkspace = useCallback(async (name: string) => {
    ...
    if (result) {
      // Call create_trip on the newly-created context to set trip metadata.
      const client = new TripClient(mero, result.lobbyContextId, result.memberPublicKey);
      try { await client.createTrip({ name }); } catch { /* init may have already set it */ }
