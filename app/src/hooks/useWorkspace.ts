...
export interface UseWorkspaceReturn {
  ...
  /**
   * Create a new unit (a fresh context of PRIMARY_SERVICE). `name` is the
   * admin-level context LABEL/alias. When the service's `init(...)` takes
   * arguments, pass `initializationParams` keyed by the Rust arg names.
   */
  createUnit: (name: string, initializationParams?: Record<string, unknown>) => Promise<string | null>;
...
export function useWorkspace(): UseWorkspaceReturn {
  ...
  const createUnit = useCallback(async (name: string, initializationParams?: Record<string, unknown>): Promise<string | null> => {
    ...
    try {
      ...
      // parameterless init(). `name` below is the context LABEL/alias, not an init arg.
