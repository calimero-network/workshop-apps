  ...
  const createNamespaceWithLobby = useCallback(
    async (alias?: string): Promise<NamespaceBootstrapResult | null> => {
      ...
      try {
        ...
        // 2b. Publish the workspace NAME as propagated group metadata. The
        // `name` passed to createNamespace is only a local alias on this node;
        // setGroupMetadata writes a CRDT MetadataRecord that every member reads
        // back via getGroupMetadata, so everyone sees the same workspace name.
        if (alias?.trim()) {
          try {
            await mero.admin.setGroupMetadata(namespaceId, { name: alias.trim() });
          } catch {
            // non-fatal: workspace still works, just falls back to local alias
          }
        }
