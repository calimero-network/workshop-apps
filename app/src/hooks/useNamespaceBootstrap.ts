...
import { SERVICE_NAME, PRIMARY_SERVICE_NAME } from '../config';

/**
 * Resolve the service name for the workspace/directory context.
 * Tries SERVICE_NAME.directory first (multi-service app); falls back to
 * PRIMARY_SERVICE_NAME for single-service apps where no 'directory' id is
 * declared in studio.config.json.
 */
function getDirectoryServiceName(): string {
  try {
    ...
    async (alias?: string): Promise<NamespaceBootstrapResult | null> => {
      ...
      try {
        ...
        const ctx = await mero.admin.createContext({
          applicationId,
          groupId: namespaceId,
          serviceName: getDirectoryServiceName(),
