...
export default function MembersPanel({ namespaceId, selfIdentity, selfRole, isAdmin, onChanged }: Props): React.ReactElement {
  ...
  // Self row only renders once selfIdentity resolves, so count it only then.
  ...
  return (
    <Wrap data-testid="members-panel">
      ...
      <List>
        {selfIdentity && (
          <MemberRow namespaceId={namespaceId ?? ''} identity={selfIdentity} role={selfRole ?? undefined} isSelf canManage={false} onRemove={remove} />
        )}
        {members.map((m) => (
