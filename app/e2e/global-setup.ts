...
// Embedded auth (core >= 0.11.0-rc.14) fails closed: a fresh node only mints
// the first root key when the first /auth/token login presents the node's
// out-of-band bootstrap secret. merod reads it from its own env, so set it here
// (spawned init/run children inherit process.env) and send the matching value
// in the first login. A fixed default is fine for these ephemeral test nodes;
// override with MERO_AUTH_BOOTSTRAP_SECRET.
const BOOTSTRAP_SECRET = process.env['MERO_AUTH_BOOTSTRAP_SECRET'] || 'calimero-e2e-bootstrap';
process.env['MERO_AUTH_BOOTSTRAP_SECRET'] = BOOTSTRAP_SECRET;

// The admin account authenticate() logs in with (user_password provider).
const ADMIN_USER = 'admin';
const ADMIN_PASSWORD = 'admin1234';
const ADMIN_PASSWORD_FILE = path.resolve(DATA_DIR, 'admin-password');
...
async function authenticate(adminUrl: string): Promise<{
  ...
  const resp = await fetch(`${adminUrl}/auth/token`, {
    ...
    body: JSON.stringify({
      ...
      provider_data: { username: ADMIN_USER, password: ADMIN_PASSWORD, bootstrap_secret: BOOTSTRAP_SECRET },
    }),
  });
...
function initNode(node: typeof NODES[0]): void {
  // merod 0.11.0-rc.17: `init --auth-mode embedded` mints the admin account at
  // init and requires its credentials up front (the password only via file/stdin/
  // env, never a bare flag). Provision the same admin/admin1234 authenticate() uses.
  writeFileSync(ADMIN_PASSWORD_FILE, ADMIN_PASSWORD);
  execSync(
