...
import { buildAuthHash, extractInvitation, initParamsFor } from './isolation.js';
...
function studioConfig(): any {
  const cfgPath = path.resolve(__dirname, '..', '..', 'studio.config.json');
  return JSON.parse(readFileSync(cfgPath, 'utf-8'));
}

function primaryServiceName(): string {
  const name = studioConfig()?.services?.[0]?.name;
...
async function provisionBase(): Promise<IsoWorkspace> {
  ...
  // A service whose init takes args rejects an empty payload, which fails every
  // isolated spec at login. Same encoding useWorkspace uses to create a context.
  const service = primaryServiceName();
  const initArgs = initParamsFor(studioConfig(), service);
  const ctx = await adminApi(node0, 'POST', '/admin-api/contexts', {
    applicationId: node0.appId,
    groupId: ns.namespaceId,
    serviceName: service,
    initializationParams: initArgs
      ? Array.from(new TextEncoder().encode(JSON.stringify(initArgs)))
      : [],
  });
...
export async function loginViaHash(page: Page, nodeIndex = 0) {
  ...
  try {
    // From a page already on the origin, a hash-only goto is a same-document
    // navigation: React never remounts, so parseAuthCallback never runs.
