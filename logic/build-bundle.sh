...
#
# Modes:
#   (default)   dev signing with a project-local Ed25519 key rather than
#               `cargo mero sign --dev`'s well-known one: the registry refuses
#               the shared dev key, and Studio re-signs this bundle to publish
#               it. The key comes from $STUDIO_DEV_SIGNING_KEY (a path to a key
#               JSON, or the key JSON inline); if unset, a key is generated once
#               at ~/.calimero-studio/studio-dev-signing.key.json and reused, so
...
# Both the build and the signature come from cargo-mero. A node refuses a
# manifest that is unsigned or whose artifact hashes are absent, so there is no
# degraded mode to fall back to when the toolchain is missing.
if ! command -v cargo-mero > /dev/null; then
  echo "Error: cargo-mero is required - run: bash ../scripts/install-cargo-mero.sh" >&2
  exit 1
fi
...
# Build each service with cargo-mero: it emits the ABI from the crate's module
# tree, compiles to wasm32, size-optimizes, and embeds the ABI as the
# `calimero_abi_v1` section the node reads for per-method flags and upgrades.
for i in $(seq 0 $((SERVICE_COUNT - 1))); do
  ...
  cargo mero build --manifest-path "crates/$SVC_NAME/Cargo.toml"
done
...
for i in $(seq 0 $((SERVICE_COUNT - 1))); do
  SVC_NAME=$(echo "$SERVICES_JSON" | jq -r ".[$i].name")
  SVC_CRATE=$(echo "$SERVICES_JSON" | jq -r ".[$i].crate")
  WASM_SRC="crates/$SVC_NAME/res/${SVC_CRATE//-/_}.wasm"
  ...
  # The node verifies each artifact's bytes against this hash before install,
  # so it is computed from the staged file rather than recorded as null.
  ENTRY_ARGS=(--arg name "$SVC_NAME"
              --arg wasm "$WASM_MEMBER"
              --argjson wsize "$(filesize "$WASM_SRC")"
              --arg whash "$(sha256 "$WASM_SRC")")
  ENTRY_FILTER='{name:$name, wasm:{path:$wasm,size:$wsize,hash:$whash}}'
  TAR_FILES="$TAR_FILES $WASM_MEMBER"

  # Declared only when the file was actually staged: a required hash cannot
  # describe an artifact that is absent from the archive.
  if [[ -f "$ABI_SRC" ]]; then
    cp "$ABI_SRC" "res/bundle-temp/$ABI_MEMBER"
    ENTRY_ARGS+=(--arg abi "$ABI_MEMBER"
                 --argjson asize "$(filesize "$ABI_SRC")"
                 --arg ahash "$(sha256 "$ABI_SRC")")
    ENTRY_FILTER="$ENTRY_FILTER + {abi:{path:\$abi,size:\$asize,hash:\$ahash}}"
    TAR_FILES="$TAR_FILES $ABI_MEMBER"
  fi

  ENTRY=$(jq -nc "${ENTRY_ARGS[@]}" "$ENTRY_FILTER")
...
if [[ "$MODE" == "release" ]]; then
  ...
  cargo mero sign res/bundle-temp/manifest.json --key "$MERO_SIGN_KEY"
else
  ...
  if [[ -n "$DEV_KEY_RAW" ]]; then
    ...
    if [[ ! -f "$DEV_KEY_PATH" ]]; then
      ...
      cargo mero key generate -o "$DEV_KEY_PATH"
      echo "Generated dev signing key at $DEV_KEY_PATH (reused on future builds)"
    fi
  fi
  cargo mero sign res/bundle-temp/manifest.json --key "$DEV_KEY_PATH"
fi
