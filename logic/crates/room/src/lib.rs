...
use chat_types::{generate_id, validate_attachment, ChatError, PublicKey as ChatPublicKey};
...
/// A shared file/image reference. The bytes live in the node's blob store
/// (uploaded by the sender, fetched by peers via `/admin-api/blobs/<id>`);
/// the message only carries this lightweight reference so the CRDT state
/// stays small and replicates cheaply.
#[derive(Debug, Clone, BorshSerialize, BorshDeserialize, Serialize, Deserialize)]
#[borsh(crate = "calimero_sdk::borsh")]
#[serde(crate = "calimero_sdk::serde")]
pub struct Attachment {
    /// Blob id returned by the node blob store on upload.
    pub blob_id: String,
    /// Original file name, for display + download.
    pub file_name: String,
    /// MIME type (e.g. `image/png`) — the frontend renders images inline.
    pub mime_type: String,
    /// Size in bytes (informational; the blob store is the source of truth).
    pub size: u64,
}
...
pub struct Message {
    ...
    pub edited: bool,
    /// Optional file/image shared with this message. `None` for plain text.
    #[serde(default)]
...
impl RoomState {
    ...
    /// Send a message that carries a shared file/image. The blob bytes are
    /// uploaded to the node blob store by the frontend first; this stores only
    /// the reference (`blob_id` + display metadata). `body` is an optional
    ...
    pub fn send_attachment(
        ...
        validate_attachment(&blob_id, &file_name, &mime_type, size)
